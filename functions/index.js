...
app.post("/swipe", async (req, res, next) => {
  try{
    ...
    await db.collection('swipe').doc().set(data);
...
app.post("/destination",  async (req, res, next) => {
  try{
    // userid, lat, lng 
        ...
        data["timestamp"] = Date.now()
    console.log(data);
    await db.collection('destination').doc().set(data);
    res.send('Record saved successfuly');
...
app.get("/getShop", async (req, res) => {
  var url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=${functions.config().google.apikey}`;
...
app.get("/getImage", async (req, res) => {
  var url = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&key=${functions.config().google.apikey}`;
